...
function actionTotal(a){
  return n(a.undb) + n(a.db) + n(a.pos);
}
function normalizeActionTotals(){
  Object.keys(data).forEach(y=>{
    data[y].actions.forEach(a=>{
      a.total = actionTotal(a);
    });
  });
}
function clearMockDataOnce(){
  const flag = "gdbAutoTotalMigrationV1";
  if(localStorage.getItem(flag)) return;
  try{
    Object.keys(localStorage).forEach(k=>{
      if(k.includes("gdbEditavelInstLeads") || k.includes("gdbMultiInstLeads")){
        localStorage.removeItem(k);
      }
    });
  }catch(e){}
  data = structuredClone(base);
  normalizeActionTotals();
  save();
...
function renderCalendar(){
  ...
  data[year].actions.forEach((a,i)=>{
    ...
    totalSpan.className="auto-total";
    totalSpan.textContent=fmt(actionTotal(a));
    totalCell.appendChild(totalSpan);
    tr.appendChild(totalCell);

    ["undb","db","pos"].forEach(k=>{
      let c=document.createElement("td");
      c.appendChild(numInput(a[k],v=>{
        a[k]=v;
        a.total=actionTotal(a);
      }));
      tr.appendChild(c);
...
function renderSummary(){
  normalizeActionTotals();
  let actions=data[year].actions;
  let sums={total:0,undb:0,db:0,pos:0};
  actions.forEach(a=>{
    sums.undb+=n(a.undb);
    sums.db+=n(a.db);
    sums.pos+=n(a.pos);
    sums.total+=actionTotal(a);
  });
  data[year].goals.forEach(g=>g.items.forEach(it=>{
    sums.undb+=n(it.undb);
    sums.db+=n(it.db);
    sums.pos+=n(it.pos);
    const splitTotal=n(it.undb)+n(it.db)+n(it.pos);
    sums.total+= splitTotal>0 ? splitTotal : n(it.total);
  }));
  totalActions.textContent=actions.length;
  totalAllLeads.textContent=fmt(sums.total);
  totalUndbLeads.textContent=fmt(sums.undb);
  totalDbLeads.textContent=fmt(sums.db);
  totalPosLeads.textContent=fmt(sums.pos);
  undbLeadAside.textContent=fmt(sums.undb)+" leads";
  dbLeadAside.textContent=fmt(sums.db)+" leads";
  posLeadAside.textContent=fmt(sums.pos)+" leads";
  undbCount.textContent=actions.filter(a=>a.inst.includes("UNDB")).length+" ações";
  dbCount.textContent=actions.filter(a=>a.inst.includes("Dom Bosco")).length+" ações";
  posCount.textContent=actions.filter(a=>a.inst.includes("Pós-UNDB")).length+" ações";
}
...
let t=localStorage.getItem("gdbThemeEditavel")||"padrao";document.querySelector(`[data-theme="${t}"]`)?.click();clearMockDataOnce();
render();
